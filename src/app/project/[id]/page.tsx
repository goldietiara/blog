...
const Project = async ({ params: { id } }: { params: { id: string } }) => {
    ...
    if (!result?.project) {
        return (
            <p className='flex items-center justify-center'>Failed to fetch projects </p>
        )
    }
                ...
                <div className="flex-1 flex items-start gap-5 w-full max-xs:flex-col">
                    ...
                    <div className="flex-1 flex items-center justify-start flex-col gap-1">
                        ...
                        <div className="flex flex-wrap whitespace-nowrap text-sm font-normal gap-2 w-full">
                            ...
                            <Link href={`/?category=${projectDetails.category}`} className="text-primary-purple font-semibold">{projectDetails ? projectDetails?.category : "Front-end"}</Link>
                        </div>
                    </div>
                </div>
